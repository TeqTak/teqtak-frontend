...
const JobSuccess = () => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        ...
        toast.success('Job link shared successfully!'); // Notify on success
      } catch (error) {
        console.error('Error sharing:', error);
        toast.error('Error sharing the job link.'); // Notify on error
      }
    } else {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(window.location.href).then(() => {
      toast.success('Job link copied to clipboard!'); // Notify on success
    }).catch(err => {
