...
// console.log("coment",comments)
  ...
  const postReply = async (commentId) => {
    try {
      const req = await fetch(`${REACT_APP_API_BASE_URL}/reply`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
  ...
  const getReply = async (reviewId) => {
    try {
      const req = await fetch(`${REACT_APP_API_BASE_URL}/reply/${reviewId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
  ...
  return (
    <React.Fragment>
      ...
      <div className="flex flex-wrap md:flex-nowrap md:w-[67%] w-[50%] md:h-[75%] h-[90%] bg-white relative shadow-lg max-[425px]:w-[80%]">
        <RiCloseLine
                    ...
                    <div key={i} className="flex flex-col gap-3 mb-4">
                      ...
                      state={{ id: value.sender.Users_PK }} className="flex gap-1 items-center">
                        <img
                          ...
                          alt="Profile"
                          className={`rounded-full w-10 h-10 ${value.sender.role === 'investor' ? 'border-[2px] border-[#FF3434] p-[1px]' : 
    value.sender.role === 'entrepreneur' ? 'border-[2px] border-[#6165F3] p-1' : 'border-[#E6E6E6] p-1'}`}
                        />
                          ...
                          {reviewReplies.map((reply, j) => (
                            <div key={j} className="flex gap-1 items-center ml-4">
                              <img
                                src={"/placeholder.jpg"}
                                alt="Profile"
                                className={`rounded-full w-10 h-10 my-1 ${value.sender.role === 'investor' ? 'border-[2px] border-[#FF3434] p-[1px]' : 
                                  value.sender.role === 'entrepreneur' ? 'border-[2px] border-[#6165F3] p-1' : 'border-[#E6E6E6] p-1' }`}
                              />
