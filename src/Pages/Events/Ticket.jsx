...
function Ticket() {
  ...
  const remainingBasic = tickets.eventTicketArray
    ? tickets.eventTicketArray.find(
        (ticket) => ticket.ticketType === "basicTicket"
      )?.quantity - add
    : 0;

  const remainingStandard = tickets.eventTicketArray
    ? tickets.eventTicketArray.find(
        (ticket) => ticket.ticketType === "standardTicket"
      )?.quantity - more
    : 0;

  const remainingPremium = tickets.eventTicketArray
    ? tickets.eventTicketArray.find(
        (ticket) => ticket.ticketType === "premiumTicket"
      )?.quantity - able
    : 0;
  ...
  return (
    <div className="main bg-white h-full w-full">
      ...
      <div className="h-[90%] overflow-y-scroll Podcast_Top_Videos">
        ...
        <div className="flex justify-center max-[425px]:flex-col max-[425px]:items-center gap-3 Podcast_Top_Videos mt-3 lg:w-[90%] w-[95%] mx-auto Ticket_Tickets">
          
          {tickets.eventTicketArray &&
            tickets.eventTicketArray.some(
              (ticket) =>
                ticket.ticketType === "basicTicket" && ticket.price > 0
            ) && (
              <div className="border rounded shadow-lg w-[33%] text-center px-4 py-6 ticket flex-shrink-0">
                ...
                <p className="pt-6">
                  Remaining: {isNaN(remainingBasic) ? 0 : remainingBasic}
                </p>
              </div>
            )}
          ...
          {tickets.eventTicketArray &&
            tickets.eventTicketArray.some(
              (ticket) =>
                ticket.ticketType === "standardTicket" && ticket.price >0
            ) && (
          ...
          {tickets.eventTicketArray &&
            tickets.eventTicketArray.some(
              (ticket) =>
                ticket.ticketType === "premiumTicket" && ticket.price > 0
            ) && (
              <div className="border rounded shadow-lg w-[33%] text-center px-4 pt-4 ticket flex-shrink-0">
