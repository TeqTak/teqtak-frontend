...
function MySubscribers() { 
  ...
  // Define the isBlocked function
  const isBlocked = (blockedId) => {
    return blockedUsers.some(blocked => blocked.blockedId === blockedId);
  };
                ...
                <div className="flex items-center gap-3">
                  <Link to="/userprofile">
                    <img
                      ...
                      alt="user profile"
                      className={`h-[40px] w-[40px] lg:h-[50px] lg:w-[50px] rounded-full ${subsc.user.role === 'investor' ? 'border-[3px] border-[#FF3434] p-[1px]' : 
    subsc.user.role === 'entrepreneur' ? 'border-[3px] border-[#6165F3] p-[1px]' : 'border-[#E6E6E6] p-1'}`}
                    />
                  </Link>
                  <div>
